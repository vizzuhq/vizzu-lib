...
const static auto tests =
    ...
    | "construct_empty" |
		...
		<=> [] (interface* df)
		{
			...
			throw_ ->* [&df] { return df->set_aggregate({}, {}); };
			throw_ ->* [&df] { return df->set_sort({}, {}, {}); };
	...
	| "cannot finalize contains same dim" |
		[] (interface* df = setup{.dims = {"d1", "d2"}, .data={
				{{"dx0", "dm0"}},
				{{"dx0", "dm0"}}
		}}) {
			throw_ ->* [&df] { df->finalize(); };
		}
	...
	| "sort dimension" |
		[] (interface* df = setup{{"d1", "d2"}, {"m1"}, {
			...
			assert ->* df->get_categories("d1") == std::array{"dx0", "dx1", "dx2"};
			check ->* df->get_categories("d2") == std::array{"dm2", "dm0", "dm1"};
			...
			check ->* std::get<std::string_view>(df->get_data(std::size_t{10}, "d1")).data() == nullptr;

			check ->* std::isnan(std::get<double>(df->get_data(std::size_t{0}, "m1")))
				== "is nan"_is_true;
			check ->* df->get_data(std::size_t{1}, "m1") == 10.25;
			check ->* df->get_data(std::size_t{2}, "m1") == 4.25;

			check ->* std::isnan(std::get<double>(df->get_data(std::size_t{3}, "m1")))
				== "is nan"_is_true;
	...
	| "another sort example" |
		[] (interface *df = setup{{"d1", "d2"}, {"m1"}, {
			...
			df->set_sort([](const record_type &lhs, const record_type &rhs) {
				auto l = (std::get<std::string_view>(lhs.getValue("d2"))[2] - '0')
						+ std::get<double>(lhs.getValue("m1"));
				auto r = (std::get<std::string_view>(rhs.getValue("d2"))[2] - '0')
			...
			assert ->* df->get_categories("d1") == std::array{"dx2", "dx1", "dx0"};
			check ->* df->get_categories("d2") == std::array{"dm2", "dm0", "dm1", "dm8"};

			check ->* std::get<std::string_view>(df->get_data(std::size_t{0}, "d1")).data() == nullptr;
			...
			check ->* std::isnan(std::get<double>(df->get_data(std::size_t{7}, "m1")))
				== "is nan"_is_true;

			check ->* df->get_data(std::size_t{8}, "m1") == 10.25;
			check ->* df->get_data(std::size_t{9}, "m1") == 4.25;
			check ->* std::isnan(std::get<double>(df->get_data(std::size_t{10}, "m1")))
				== "is nan"_is_true;
		}
	...
	| "sort measure" |
		[] (interface* df = setup{{"d1"}, {"m1"}, {
			...
			check ->* std::isnan(std::get<double>(df->get_data(std::size_t{9}, "m1")))
				== "is nan"_is_true;
		}
