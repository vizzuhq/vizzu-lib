...
class ManualClient {
  ...
  populateLibs() {
    return fetch("/getLibs")
      ...
      .then((data) => {
        const testLibs = Object.entries(data).map(
          ([name, url]) => new TestLib(url, name),
        );
        ...
        const lastSelectedUrl =
          data[this.url.getQueryParam("vizzuUrl")] ||
          localStorage.getItem("vizzuUrl");
        this.vizzuUrl.value = lastSelectedUrl;
        if (!this.vizzuUrl.value) this.vizzuUrl.value = data["localhost"];

        const lastSelectedRefUrl =
          data[this.url.getQueryParam("vizzuRefUrl")] ||
          localStorage.getItem("vizzuRef");
        this.vizzuRef.value = lastSelectedRefUrl;
        if (!this.vizzuRef.value)
          this.vizzuRef.value = data["HEAD"] || ldata["localhost"];
      });
  }
  ...
  populateCases() {
    return fetch("/getTests")
      ...
      .then((data) => {
        ...
        if (!this.testCase.value)
          this.testCase.value = JSON.stringify(testCases[0]);
      });
  }
  ...
  update() {
    ...
    if (this.vizzuUrl.value !== this.vizzuRef.value) {
      ...
      const imgDiff = new window.ImgDiff(
        this.frame,
        this.frameRef,
        this.difCanvas,
      );
  ...
  setupButtons() {
    ...
    this.validate.addEventListener("click", () =>
      TestCase.validate(this.testCase),
    );
  }
  ...
  connectSliders() {
    ...
    return Promise.all([waitForLoad, waitForLoadRef])
      ...
      .then((setups) => {
        const slider =
          this.frame.contentWindow.document.getElementById("myRange");
        const sliderRef =
          this.frameRef.contentWindow.document.getElementById("myRange");
  ...
  openSelectedInVSCode() {
    const testCaseObject = JSON.parse(this.testCase.value);
    const testCasePath =
      testCaseObject.testRepo + testCaseObject.testFile + ".mjs";
