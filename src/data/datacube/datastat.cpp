				   ...
				   const DataFilterStack &filter)
{
	...
	for (const auto &idx : indices)
	{
		...
		{
			auto valueCnt = table.getInfo(idx.getColIndex()).discreteValueCnt();
			usedColumnIDs.insert({ (size_t)idx.getColIndex(), usedValues.size() });
...
void DataStat::countValues()
{
	for (const auto &values: usedValues) {
		auto cnt = 0;
		for (auto used: values) if (used) cnt++;
