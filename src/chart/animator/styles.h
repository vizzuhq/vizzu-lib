...
template<class T>
class StyleMorph<T, std::void_t<
	std::enable_if_t<Type::isoptional<T>::value>,
	decltype(interpolate(*std::declval<const T&>(), *std::declval<const T&>(), double{}))
>>
...
public:
	...
	template <typename T>
	auto operator()(const T &source, const T &target, T &actual) 
