...
class SpecNameEnum(ScaleExtremaType)
    (absolute, relative, minOffset, maxOffset)
    (,%,min,max);

class ScaleExtrema : public Type::PhysicalValue<double, ScaleExtremaType>
{
public:
...
public:
    ...
    Math::Range<double> getRange(const Math::Range<double> &original) const;
