...
template <typename B> class ElapsedTime
{
public:
	ElapsedTime()
	{
		if (currentSw) measure(*currentSw);
	}
	template <class T> ElapsedTime(Stopwatch<T> &activeSw)
	{
		if (currentSw) measure(*currentSw);
	...
	{
		auto diff =
		    std::chrono::high_resolution_clock::now() - overallStart;
...
protected:
	...
	void measure(TaggedTimePoint &ttp)
	{
		...
		auto tmp =
		    std::chrono::high_resolution_clock::now() - ttp.start;
		auto time =
		    std::chrono::duration_cast<std::chrono::microseconds>(
		        tmp);
		if (item == timeTable.end())
			timeTable.insert(
			    std::make_pair(ttp.tag, item_t{1, time}));
		else
			item->second.time += time, item->second.count++;
		overallDuration += time;
		if (currentSw) currentSw->reset();
		currentSw = nullptr;
	}
};
...
template <typename B>
std::map<std::string, typename ElapsedTime<B>::item_t>
    ElapsedTime<B>::timeTable;
template <typename B>
std::chrono::high_resolution_clock::time_point
    ElapsedTime<B>::overallStart;
