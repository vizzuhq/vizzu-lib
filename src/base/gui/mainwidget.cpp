...
DragObjectPtr MainWidget::onPointerDown(
...
bool MainWidget::onPointerUp(const GUI::PointerEvent &event,
    DragObjectPtr)
{
...
bool MainWidget::onPointerMove(const GUI::PointerEvent &event,
    DragObjectPtr &)
{
	setCursor(GUI::Cursor::point);

	bool res = false;
	if (dragObject) { res |= dragObject->dragMoved(event.pos); }
