...
export default class UnPivot {
    ...
    static convert(data) {
        ...
        for (let i = 0; i < data.dimensions.length; i++) {
            ...
            assert(typeof item === 'object' && item !== null && !Array.isArray(item), 'data.dimensions.item is not an object');
            assert('name' in item, 'data.dimensions.item.name is requreid');
            assert(item.type === undefined || item.type === 'dimension', 'data.dimensions.item.type is invalid');
