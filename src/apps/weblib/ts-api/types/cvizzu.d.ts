...
interface CVizzu {
	...
	_vizzu_pointerDown(chart: CChartPtr, canvas: CCanvasPtr, pointerId: number, x: number, y: number): void
	_vizzu_pointerUp(chart: CChartPtr, canvas: CCanvas, pointerId: number, x: number, y: number): void
	_vizzu_pointerMove(chart: CChartPtr, canvas: CCanvas, pointerId: number, x: number, y: number): void
	...
	_vizzu_update(chart: CChartPtr, canvas: CCanvas, width: number, height: number, renderControl: number): void
	...
	_data_addMeasure(chart: CChartPtr, name: CString, unit: CString, values: CArrayPtr, count: number): void
	...
	_chart_setFilter(chart: CChartPtr,
		filter: CFunction<(CRecordPtr) => boolean>,
		deleter: CFunction<(filter: CFunction<(CRecordPtr) => boolean>) => void>): void
	...
	_addEventListener(chart: CChartPtr, name: CString, callback: CFunction<(CEventPtr, CString) => void>): void
	_removeEventListener(chart: CChartPtr, name: CString, callback: CFunction<(CEventPtr, CString) => void>): void
